...
async function loadMisuAging(){
  ...
  try{
    ...
    if(!d.ok){ if(section) section.innerHTML = '<div style="color:var(--r);padding:10px">분석 실패</div>'; return; }
...
function renderMisuAging(aging, 기준일){
  ...
  const summaryHtml = `
    <div style="background:var(--card2);border-radius:12px;padding:13px;margin-bottom:12px">
      <div style="font-size:10px;color:var(--t3);font-weight:700;letter-spacing:.1em;margin-bottom:10px">📊 미수 연령 요약 (기준일: ${기준일})</div>
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px">
        ...
        <div style="background:rgba(88,166,255,.08);border:1px solid rgba(88,166,255,.2);border-radius:10px;padding:10px;text-align:center">
          <div style="font-size:9px;color:var(--b);font-weight:700;margin-bottom:4px">31~60일</div>
          <div style="font-family:'JetBrains Mono',monospace;font-size:14px;font-weight:700;color:var(--b)">${total60.toLocaleString()}원</div>
        </div>
  ...
  const rowsHtml = aging.map(r => {
    // 위험도 색상
    const risk = r.d90plus > 0 ? 'var(--r)' : r.d90 > 0 ? 'var(--y)' : r.d60 > 0 ? 'var(--b)' : 'var(--g)';
    const riskLabel = r.d90plus > 0 ? '⚠️ 90일↑' : r.d90 > 0 ? '61-90일' : r.d60 > 0 ? '31-60일' : '30일↓';

    // 연령 바
    const barTotal = r.총미수;
    const b30  = Math.round(r.d30/barTotal*100);
    const b60  = Math.round(r.d60/barTotal*100);
    const b90  = Math.round(r.d90/barTotal*100);
    const b90p = 100-b30-b60-b90;

    return `
    <div style="background:var(--card);border:1px solid var(--border);border-radius:12px;padding:13px;margin-bottom:8px" onclick="openMisuDetail('${r.거래처}')" style="cursor:pointer">
      <div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:8px">
        <div>
          <div style="font-size:14px;font-weight:700;cursor:pointer">${r.거래처}</div>
      ...
      <div style="height:6px;border-radius:3px;overflow:hidden;display:flex;gap:1px;margin-bottom:8px">
        ...
        ${b60>0  ? `<div style="width:${b60}%;background:var(--b);border-radius:3px"></div>` : ''}
      ...
      <div style="display:grid;grid-template-columns:1fr 1fr 1fr 1fr;gap:4px;font-size:10px;text-align:center">
        ...
        ${r.d60>0    ? `<div style="color:var(--b)">~60일<br><b>${r.d60.toLocaleString()}</b></div>` : '<div style="color:var(--t3)">~60일<br>-</div>'}
...
function openMisuLimitModal(code, name, currentLimit){
  ...
  modal.style.cssText = 'position:fixed;inset:0;background:rgba(0,0,0,.85);z-index:200;display:flex;align-items:flex-end;justify-content:center';
  modal.innerHTML = `
    <div style="background:var(--card);border-radius:20px 20px 0 0;padding:22px 18px 44px;width:100%;max-width:440px">
      ...
      <div style="display:flex;flex-direction:column;gap:4px;margin-bottom:16px">
        ...
        <input id="misu-limit-input" type="number" inputmode="numeric" value="${currentLimit||0}"
          style="background:var(--card2);border:1.5px solid var(--border);border-radius:10px;padding:12px 13px;color:var(--t);font-size:16px;font-family:'JetBrains Mono',monospace;outline:none">
      </div>
      <!-- 빠른 선택 버튼 -->
      ...
      <div style="display:flex;gap:8px">
        ...
        <button onclick="saveMisuLimit('${code}','${name}')"
...
// ── renderStores 오버라이드 (비활성/한도/aging 버튼 추가) ────────

const _origRenderStores = typeof renderStores === 'function' ? renderStores : null;
...
function renderStores(){
  ...
  if(!list.length){
    el.innerHTML='<div class="empty"><div class="empty-icon">🔍</div><div style="font-size:13px">거래처 없음</div></div>';return;
  }
  ...
  el.innerHTML = list.map(s => {
    ...
    return `<div class="store-card" style="${isInactive?'opacity:.5':''}">
      <div style="display:flex;justify-content:space-between;align-items:flex-start">
        <div style="flex:1">
          <div class="store-name">${s.이름||''}
            ${isInactive?'<span style="font-size:9px;background:rgba(255,107,107,.15);color:var(--r);padding:2px 6px;border-radius:4px;margin-left:6px">비활성</span>':''}
          </div>
        ...
        <div style="display:flex;flex-direction:column;gap:4px;margin-left:8px">
          <div style="display:flex;gap:4px">
            <button class="btn-edit" onclick="openStoreEdit('${s.코드}')">수정</button>
            <button onclick="openMisuLimitModal('${s.코드}','${(s.이름||'').replace(/'/g,'\'')}',${한도})"
          ...
          <button onclick="toggleStoreStatus('${s.코드}','${(s.이름||'').replace(/'/g,'\'')}','${s.상태||'활성'}')"
            style="padding:5px 8px;background:${isInactive?'rgba(63,185,80,.1)':'rgba(255,107,107,.08)'};border:none;border-radius:7px;color:${isInactive?'var(--g)':'var(--r)'};font-size:10px;font-weight:600;cursor:pointer;font-family:'Noto Sans KR',sans-serif">
            ${isInactive?'활성화':'비활성화'}
          </button>
        </div>
      </div>
...
// ── 필터 버튼에 비활성 추가 (goTab 오버라이드로 처리) ──────────
// 거래처탭 진입 시 비활성 필터 버튼 동적 추가
...
function goTab(t){
  ...
  if(t==='dashboard') silentRefresh();
  if(t==='history'){const hm=document.getElementById('hist-month');const dm=document.getElementById('dash-month').value;if(!hm.value)hm.value=dm;loadHistoryAll();}
  if(t==='stores'){
    // 비활성 필터 버튼 추가 (없으면)
...
function insertAgingSection(){
  ...
  wrapper.innerHTML = `
    ...
    <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:10px">
      ...
      <button onclick="loadMisuAging()" style="padding:6px 12px;background:var(--card);border:1px solid var(--border);border-radius:8px;color:var(--t2);font-size:11px;cursor:pointer;font-family:'Noto Sans KR',sans-serif">🔄 새로고침</button>
    </div>
...
async function init(){
  ...
  document.getElementById('dash-month').value=ym;
  const todayStr=now.toISOString().slice(0,10);
  ['nw-date','np-date'].forEach(id=>{const el=document.getElementById(id);if(el){el.value=todayStr;el.min=todayStr;}});
  document.getElementById('hist-month').value=ym;
  await loadAll();
  // aging 섹션 삽입
  setTimeout(insertAgingSection, 500);
}