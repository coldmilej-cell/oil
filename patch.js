...
// ══ 내역탭 일자 버튼 ══
function renderHistDayBtns(selectedDay){
  const el = document.getElementById('hist-day-btns');
  if(!el) return;
  const month = document.getElementById('hist-month-sel')?.value || '';
  if(!month){ el.innerHTML=''; return; }
  // 해당 월의 납품 있는 날짜 추출
  const days = [...new Set(
    (typeof txList !== 'undefined' ? txList : [])
      .filter(t => String(t.날짜||'').startsWith(month))
      .map(t => String(t.날짜||'').slice(8,10))
  )].sort();
  el.innerHTML = ['전체',...days].map(d => {
    const isAll = d==='전체';
    const active = isAll ? !selectedDay : d===selectedDay;
    return `<div onclick="selectHistDay('${d}')"
      style="padding:3px 8px;border-radius:6px;font-size:11px;cursor:pointer;
      background:${active?'var(--p2)':'var(--card)'};
      color:${active?'#fff':'var(--t2)'};border:1px solid ${active?'var(--p2)':'var(--border)'};"
    >${isAll?'전체':d+'일'}</div>`;
  }).join('');
}

let _histSelectedDay = '';

function selectHistDay(d){
  _histSelectedDay = d==='전체' ? '' : d;
  renderHistDayBtns(_histSelectedDay);
  renderHistory();
}
